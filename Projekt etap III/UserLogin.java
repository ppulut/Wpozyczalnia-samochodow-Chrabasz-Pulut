    ...
    public UserLogin(Client.ClientWorker clientsocket) {
        ...
        registerButton.addActionListener(ae -> {
                ...
                if(userCreatePassword.equals(userConfirmPassword)) {
                    if (userCreatePassword.length() <= 3 || userLogin.length() <= 3 || userLogin.length()>20 || userCreatePassword.length()>20) {
                        JOptionPane.showMessageDialog(null, "Haslo oraz login musza miec pomiedzy 4 a 20 znakow");
                    } else {
